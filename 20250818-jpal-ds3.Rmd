...
```
# ------------------------- Logging ------------------------------------------
# Create a dated log file in data/derived/logs/
```{r}
log_dir = here::here("data", "derived", "logs")

if (!dir.exists(log_dir)) dir.create(log_dir, recursive = TRUE)

log_file = file.path(log_dir, paste0("build_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".log"))
...
BUFFER_RADIUS_M      = 1852  # ~1 arc-minute at mid-latitudes; you stated 1,852 m
...
I anonymise locations by keeping only the $H3$ index at resolution $8$. $H3$’s official tables report an average edge length \approx $531$ m and area \approx $0.737$ $km^2$  for res-8 cells. Therefore, the precise position of any request is coarsened to a hex region with radius \approx $531$ m (apothem \approx $460$ m), ensuring spatial precision no finer than ~500 m while preserving neighbourhood-scale signal.Lastly, I anonymise the exact timestamps (down to the second) by binning the requests into 6-hour windows. 
